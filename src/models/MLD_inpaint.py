...
class MLD_Inpaint(MG):
    """ 
    
    Multimodal Latent Diffusion model version 0.1
    ...
    def __init__(self, aes,  train_loader,
        ...
        super(MLD_Inpaint, self).__init__(
    ...
    def compute_loss(self, x, eps = 1e-5):
        ...
        with torch.no_grad():
            ...
            if self.global_step ==0:
                ...
                self.stat_up = {}
                self.stat_up["cat"] = {}
                self.stat_up["cat"]["std"] = x_0.std()
                self.stat_up["cat"]["mean"] = x_0.mean()
                self.stat_up["cat"]["min"] = x_0.min()
                self.stat_up["cat"]["max"] = x_0.max()
                for key in x_st.keys():
                    self.stat_up[key] = {}
                    self.stat_up[key]["std"] = x_st[key].std()
                    self.stat_up[key]["mean"] = x_st[key].mean()
                    self.stat_up[key]["min"] = x_st[key].min()
                    self.stat_up[key]["max"] = x_st[key].max()
        ...
        loss = self.sde.train_step(x_0,self.score) 
    ...
    def sample(self, N):
       ...
       # self.sde_reverse.device = self.device
        with torch.no_grad():
            ...
            if self.use_ema:
                x_c = self.sde.mod_repaint( score_net= self.model_ema.module,
                        x = x_c , mask= torch.zeros_like(x_c), debug=None
                    )
            else:
                x_c = self.sde.mod_repaint( score_net= self.model_ema.module,
                        x = x_c , mask= torch.zeros_like(x_c), debug=None
        ...
        with torch.no_grad():
            ...
            for idx, s_key in enumerate(self.subset_list_dict.keys()):
                if len(self.modalities_list) != len(self.subset_list_dict[s_key]):
                    ...
                    x_subset = x_0 * mask + torch.randn_like(x_0) * (1. - mask) 
                    #encodings_subset = self.normalize( deconcat(x_subset, self.modalities_list) )
                    #x_subset = concat_vect(encodings_subset)
                    if self.use_ema:
                        x_c = self.sde.mod_repaint( score_net= self.model_ema.module,
                            x = x_subset , mask= mask, debug=None
                        )
                    else:
                        x_c = self.sde.mod_repaint( score_net= self.score,
                            x = x_subset , mask= mask, debug=None
                        )
                else:
                    x_c = x_0
    ...
    def standerdize(self, encodings):
        if self.preprocess_type =="latent":
            encodings = (encodings) /(self.stat["cat"]["std"] ) 
        else:
