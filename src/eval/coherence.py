...
from src.eval_metrics.restoration_met.psnr import *
...
def test_restoration(model, subset_list, modalities_list,d_loader,batch_size ,num_samples_fid, device ,path_fid ,do_fd, do_psnr, nb_batchs = None,noise_level=0.0,ts=None):
    modalities_str = np.array([mod.name for mod in modalities_list])
    subsets = { ','.join( modalities_str[s]) : s for s in subset_list}
    ...
    if do_psnr:
       # inception = get_inception_net()
        gen_perf_psnr = dict();
        for k, s_key in enumerate(subsets.keys()):
            gen_perf_psnr[s_key] = dict();
            for i, mod in enumerate(modalities_list):
                gen_perf_psnr[s_key][mod.name] = init_metric_psnr()
       # gen_perf_fd["random"]= dict()           
       # for i, mod in enumerate(modalities_list):
        #        if mod.gen_quality:
        #            gen_perf_fd["random"][mod.name] = init_metric_fd(mod.fd["act_dim"],limit=num_samples_fid,device=device)
 ...
 #   print(gen_perf_fd)
   ...
   # gen_perf['random'] = [];
    ...
    for iteration, batch in tqdm( enumerate(d_loader), desc =" Executing Coherence Evaluation" ):
        ...
        output_cond_gen = mm_vae.conditional_restore_all_subsets(batch_d,noise_level = noise_level, ts =ts) 
        ...
        for k, s_key in enumerate(output_cond_gen.keys()):
            ...
            if (batch_size*iteration) < num_samples_fid and do_fd:
                ...
                for i, mod in enumerate(modalities_list):
                    if mod.gen_quality:
                        populate_metrics_psnr( gen_perf_psnr[s_key][mod.name] ,x =batch_d[mod.name] ,y = output_cond_gen[s_key][mod.name])
    ...
    psnr_results = dict()  
    for k, s_key in enumerate(subsets.keys()):
            psnr_results[s_key] = dict();
            for i, mod in enumerate(modalities_list):
                    psnr_results[s_key][mod.name] = gen_perf_psnr[s_key][mod.name].compute()
 ...
 #   gen_perf['random'] = mean_eval_metric(gen_perf['random'])
    ...
    return gen_perf,fd_results,psnr_results