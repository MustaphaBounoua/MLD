...
class Encoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
                 ...
                 resolution, z_channels, double_z=False, use_linear_attn=False, attn_type="vanilla",embed_dim = 64,
                 **ignore_kwargs):
        super().__init__()
        ...
        self.quant_conv = torch.nn.Conv2d(2 * z_channels, 1 * embed_dim, 1)
...
class Decoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
                 attn_resolutions, dropout=0.0, resamp_with_conv=True, in_channels,
                 resolution, z_channels, give_pre_end=False, tanh_out=False, use_linear_attn=False,embed_dim =64,
        ...
        self.curr_res =curr_res = resolution // 2**(self.num_resolutions-1)
    ...
    def forward(self, z):
        ...
        z = z.view(z.size(0),self.z_channels,self.curr_res,self.curr_res )
...
def main():
    ...
    z = encoder(torch.randn(16,3,64,64))
    print(encoder)
    print(z.shape)
    out = decoder(z)
    print(out.shape)
