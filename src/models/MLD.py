...
from src.components.mm_sde import VP_SDE
...
import os
import pickle
from src.components.ema import EMA
...
class MLD(MG):
    ...
    Multimodal Latent Diffusion model 
    ...
    def test_step(self, batch, batch_idx):
        ...
        log_results_train_step(self.logger, results,
                               self.global_step, prefix="test/")
    ...
    def compute_loss(self, x):
        ...
        with torch.no_grad():
              ...
              #  x_st = deconcat(x_0,modalities_list= self.modalities_list)
                ...
                if self.stat == None:
                    for key in encodings.keys():
                        ...
                        self.stat_up[key]["std"] = encodings[key].std(
                            dim=0).detach()
                        self.stat_up[key]["mean"] = encodings[key].mean(
                            dim=0).detach()
                      #  self.stat_up[key]["min"] = encodings[key].min().detach()
                     #  self.stat_up[key]["max"] = encodings[key].max().detach()
                        print(key+"std")
                        print(encodings[key].std().clone().detach())
                        print(key+"mean")
                        print(encodings[key].mean().clone().detach())
            ...
            if self.global_step == 0:
                ...
                for key in x_st.keys():
                    self.stat_up[key] = {}
                    self.stat_up[key]["std"] = x_st[key].std().detach()
                    self.stat_up[key]["mean"] = x_st[key].mean().detach()
                    self.stat_up[key]["min"] = x_st[key].min().detach()
                    self.stat_up[key]["max"] = x_st[key].max().detach()
                print("after all preprocess")
                print(self.stat_up)

        loss, subset = self.sde.train_step(x_0, self.score, nb_mods=len(self.modalities_list), d=self.d,
                                           subset_list_mask=self.mask_subset)
        ...
        if subset != None:
            dim_sub = 0.0
            for i, mod in enumerate(self.modalities_list):
                if i in subset:
                    dim_sub += mod.latent_dim
            loss = loss * (1 + dim_sub/(self.latent_dim - dim_sub))

        for mod in self.modalities_list:
            losses[mod.name] = loss_mod[mod.name].sum().detach() / \
                self. batch_size
    ...
    def sample(self, N):
       ...
       # self.sde_reverse.device = self.device
        with torch.no_grad():
            ...
            if self.use_ema:
                x_c = self.sde.joint_gen(
                    score_net=self.model_ema.module, x=x_c)
            else:
    ...
    def conditional_gen_latent_subsets(self, x, N=None):
        self.score.eval()
        if self.use_ema:
            self.model_ema.module.eval()
        results = {}
        with torch.no_grad():
            ...
            for idx, s_key in enumerate(self.subset_list_dict.keys()):
                if len(self.modalities_list) != len(self.subset_list_dict[s_key]):
                    ...
                    x_subset = x_0 * mask + \
                        torch.randn_like(x_0) * (1.0 - mask)

                    if self.use_ema:
                        # x_c = self.sde.mod_repaint(score_net = self.score, x = x_subset , mask= mask ,subset=self.mask_subset[idx]["subset"])
                        x_c = self.sde.mod_cross_gen(
                            score_net=self.model_ema.module, x=x_subset, mask=mask, subset=self.mask_subset[idx]["subset"])
                    else:
                        x_c = self.sde.mod_cross_gen(
                            score_net=self.score, x=x_subset, mask=mask, subset=self.mask_subset[idx]["subset"])
                ...
                if self.preprocess_type == "modality":
                    z_mods = self.postprocess(z_mods)
                results[s_key] = [concat_vect(z_mods).clone().detach()]
    ...
    def normalize(self, encodings):
        if self.preprocess_type == "latent":
            encodings = (encodings - self.stat["cat"]["min"]) / \
                (self.stat["cat"]["max"] - self.stat["cat"]["min"])
        else:

            for key in encodings.keys():
                encodings[key] = (encodings[key] - self.stat[key]["min"]) / \
                    (self.stat[key]["max"] - self.stat[key]["min"])
        return encodings

    def denormalize(self, encodings):
        if self.preprocess_type == "latent":
            encodings = (
                encodings * (self.stat["cat"]["max"] - self.stat["cat"]["min"])) + self.stat["cat"]["min"]
        else:
            for key in encodings.keys():
                encodings[key] = (encodings[key] * (self.stat[key]["max"] -
                                  self.stat[key]["min"])) + self.stat[key]["min"]
        return encodings
    ...
    def standerdize(self, encodings):
        if self.preprocess_type == "latent":
            encodings = (
                encodings - self.stat["cat"]["mean"]) / (self.stat["cat"]["std"])
        else:
            for key in encodings.keys():
                self.stat[key]["mean"] = self.stat[key]["mean"].to(
                    encodings[key].device)
                self.stat[key]["std"] = self.stat[key]["std"].to(
                    encodings[key].device)
                encodings[key] = (encodings[key] - self.stat[key]
                                  ["mean"]) / (self.stat[key]["std"])
    ...
    def destanderdize(self, encodings):
        if self.preprocess_type == "latent":
            encodings = (
                encodings * (self.stat["cat"]["std"])) + self.stat["cat"]["mean"]
        else:
            for key in encodings.keys():
                self.stat[key]["mean"] = self.stat[key]["mean"].to(
                    encodings[key].device)
                self.stat[key]["std"] = self.stat[key]["std"].to(
                    encodings[key].device)
                encodings[key] = (
                    encodings[key] * (self.stat[key]["std"])) + self.stat[key]["mean"]
                # encodings[key] = (encodings[key]   * (self.stat[key]["std"] ) )
